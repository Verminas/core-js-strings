 ...
 */
// return ind > 0 ? str.slice(0, ind) + str.slice(ind + len) : str;
function removeFirstOccurrences(str, value) {
  // throw new Error('Not implemented');
  const ind = str.indexOf(value);
  return ind > -1 ? str.replace(value, '') : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  return ind > -1 ? str.slice(0, ind) + str.slice(ind + len) : str;
}
...
* const arr = str.split('');
  ...
  const arrNum = arr.map((char) => char.charCodeAt());
  sum = arrNum.reduce((summ, current) => summ + current, 0);
  return sum;
* */
function sumOfCodes(str) {
  ...
  let sum = 0;
  let index = 0;
  if (str === null || str === undefined) return sum;
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  const regExp = /[,!?\s]/g;
  const newStr = str.trim().toLowerCase().replace(regExp, '');
  const indexStop = Math.trunc(newStr.length / 2);
  let count = 0;
  let j = -1;
  for (let i = 0; i < indexStop; i += 1) {
    if (newStr[i] === newStr.at(j)) count += 1;
    j -= 1;
  }
  return count === indexStop;
}
